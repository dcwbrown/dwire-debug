...
int GetBreakResponseByte(int breaklen) {
  ...
  //Ws(", skipping [");
  if    (byte == 0)    {/*Wc('0');*/ byte = ReadByte();} // Skip zero bytes generated by break.
  if    (byte == 0)    {/*Wc('0');*/ byte = ReadByte();} // Skip zero bytes generated by break.
  if    (byte == 0)    {/*Wc('0');*/ return 0;} // This many zeroes means we're very much faster than the chip
...
int scaleby(int byte) {
  ...
  Wc(' '); Wd(remaining,1); Wc(' '); for (int j=0; j<9; j++) Wd(lengthcount[j],1);
