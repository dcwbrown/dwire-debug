...
int GetBreakResponseByte(int breaklen) {
  ...
  while (byte == 0)    {byte = ReadByte();} // Skip zero bytes generated by break.
  while (byte == 0xFF) {byte = ReadByte();} // Skip 0xFF bytes generated by line going high following break.
...
int TryBaudRate(int baudrate, int breaklen) {
  ...
  Ws("Trying "); Ws(UsbSerialPortName); Ws(", baud rate "); Wd(baudrate,5);
...
void FindBaudRate() {
  ...
  int baudrate = 50000; // Start well above the fastest dwire baud rate based
  ...
  while (factor > 105) {
    ...
    baudrate = (baudrate * 100) / (factor-5);
    breaklen = 50000 / baudrate; // Now we have the approx byte len we can use a shorter break.
  ...
  breaklen = 30000 / baudrate; // Now we have the approx byte len we can use a shorter break.
