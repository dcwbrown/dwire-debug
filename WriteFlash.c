...
// Slow flash writing - used when there is no helper code already present in flash.


void WriteFlashBufferWord(u16 w) {
  // Assumes r29=1, Z=addr to write
  // Uses    r0, r1, r30, r31
  // Increments Z by 2
  DwWrite(ByteArrayLiteral(
    ...
    0xD2, 0xBF, 0xD7, 0x23,        // out SPMCSR,r29 = 01 = SPMEN
...
void WriteFlashBufferXX(u16 a, u8 *buf) {

  DwWrite(ByteArrayLiteral(
  	0x66,
    0xD0, 0, 29, 0xD1, 0, 32,         // Set PC=29, BP=32 - address registers r29 through r31
    0xC2, 5, 0x20, 1, lo(a), hi(a),   // r29=op (1), Z=first byte address of page
    0xD0, 0x1F, 0                     // Set PC=$1F00, inside the boot section to enable spm
  ));

	for (int i=0; i<32; i++) {
		Wc('.'); Flush();
		WriteFlashBufferWord(buf[2*i] | (buf[2*i+1] << 8));
	}
...
void WriteFlashBuffer(u16 a, u8 *buf) {
	Wc('['); Flush();
  u8 *p = buf;
  u8* q = FlashWriteBuffer;
  memcpy(q, ByteArrayLiteral(0x66, 0xD0, 0x1F, 0x00));	           q+=4;  // Set PC to bootsection for spm to work
  memcpy(q, ByteArrayLiteral(0xD0, 0, 29, 0xD1, 0, 32));	         q+=6;  // Set PC=29, BP=32 - address registers r29 through r31
  memcpy(q, ByteArrayLiteral(0xC2, 5, 0x20, SPMEN, lo(a), hi(a))); q+=6;  // r29=op (1), Z=first byte address of page
  for (int i=0; i<PageSize()/2; i++) {
    memcpy(q, ByteArrayLiteral(0xD2, 0xB4, 0x02, 0x23)); q+=4; *(q++) = *(p++); // in r0,DWDR (low byte)
    memcpy(q, ByteArrayLiteral(0xD2, 0xB4, 0x12, 0x23)); q+=4; *(q++) = *(p++); // in r1,DWDR (high byte)
    memcpy(q, ByteArrayLiteral(0xD2, 0xBF, 0xD7, 0x23)); q+=4;                  // out SPMCSR,r29 = 01 = SPMEN
    memcpy(q, ByteArrayLiteral(0xD2, 0x95, 0xE8, 0x23)); q+=4;                  // spm
    memcpy(q, ByteArrayLiteral(0xD2, 0x96, 0x32, 0x23)); q+=4;                  // adiw Z,2
  }
  memcpy(q, ByteArrayLiteral(0xD0, 0, 29, 0xD1, 0, 32));           q+=6; // Set PC=29, BP=32 - address registers r29 through r31
  memcpy(q, ByteArrayLiteral(0xC2, 5, 0x20, PGWRT, lo(a), hi(a))); q+=6; // r29=op, Z=first byte address of page
  memcpy(q, ByteArrayLiteral(0x64, 0xD2, 0xBF, 0xD7, 0x23));       q+=5; // out SPMCSR,r29 (3=PGERS, 5=PGWRT)
  memcpy(q, ByteArrayLiteral(0xD2, 0x95, 0xE8, 0x33));             q+=4; // spm
  Assert(q <= FlashWriteBuffer + sizeof(FlashWriteBuffer));
...
void ErasePage(u16 a) { // a = byte address of first word of page
  // Uses r29, r30, r31
...
void WriteFlashCommand() { // For testing
  ...
  for (int i=0; i<64; i++) {testbuf[i] = i+1;}

  WriteFlashBuffer(0, testbuf);
  //SpmFlashPage(PGWRT, 0);
