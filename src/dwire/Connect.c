...
int GetBreakResponseByte() {
  ...
  Vs(", skipping [");
  if    (byte == 0)    {Vc('0'); byte = ReadByte();} // Skip zero bytes generated by break.
  if    (byte == 0)    {Vc('0'); byte = ReadByte();} // Skip zero bytes generated by break.
  if    (byte == 0)    {Vc(']'); return 0;} // This many zeroes means we're very much faster than the chip
  while (byte == 0xFF) {Vc('F'); byte = ReadByte();} // Skip 0xFF bytes generated by line going high following break.
  Vs("]");
